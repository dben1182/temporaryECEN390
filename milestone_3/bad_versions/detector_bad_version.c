...
const static double tempPowerValues[NUM_FREQUENCIES] = {10, 1, 5999, 8, 26, 6, 17, 4, 3, 1};



// frequencies are specified as true if ignores
bool detectorIgnoredFrequencies[NUM_FREQUENCIES];
//counter used for decimation or to ignore every tenth input
uint16_t decimationCounter;

//array used to store the number of hits for each player
uint16_t detector_hitArray[NUM_FREQUENCIES];
...
//run test is a boolean flag used to do the isolated testing for the first
//passoff requirement of the lab. It is set to false by the init function, but
//is set to true by the runTest function if it is called
static bool runTest;

//flag used to determine if a hit has been detected
static bool detector_hitDetectedFlag;

static bool testingHitDetectedFlag;

//global variable that stores the current power Values
static double currentPowerValues[NUM_FREQUENCIES];
...
//test function used to hard code some values for isolated testing purposes
...
void detector(bool interruptsCurrentlyEnabled) 
{
  ...
  for(uint32_t i = FOR_LOOP_START_VALUE; i < elementCount; i++)
  {
    ...
    scaledADCValue = (rawADCValue / RAW_ADC_SCALING) - RAW_ADC_ONE_OFFSET;
    ...
    //will also run if runTest is true to bypass all that stuff
    if(decimationCounterComplete() || runTest)
    {
      //case runTest is  false, so we do all the things normally
      if(!runTest)
      {
        //runs the FIR filter
        filter_firFilter();
      ...
      if(!lockoutTimer_running())
      {
        ...
        //case run test is false
        else
        {
          //if runTest is false, we get each of the current power values
          //normally from the filter helper function anc also set channel Index
          //arrays
          for(uint16_t j = FOR_LOOP_START_VALUE; j < NUM_FREQUENCIES; j++)
          {
        ...
        //resets the decimation counter back to zero at the end of this
        resetDecimationCounter();
      }
    }
...
{
  //iterates through each value in the detector_hitArray and returns that
  //respective array
  for(uint16_t i = FOR_LOOP_START_VALUE; i < NUM_FREQUENCIES; i++)
  {
...
void detector_runTest() 
{
  ...
  //case there has been a hit detected, so we print out that there has been one detected
  //with the corresponding max value and threshold value
  if(detector_hitDetected())
  {
    printf("hit detected. Max value %f met threshold value %f\n", currentPowerValues[MAX_VALUE_INDEX],currentPowerValues[MEDIAN_INDEX_VALUE]*FUDGE_FACTOR);
  }
