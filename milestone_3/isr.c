...
// i.e. index out will be some number and the data array at that index number
// will be the oldest data value in the array

// index In refers to the index corresponding to the index that is one past the
// newest value. i.e., if the index of the newest value is 6, the value of
// indexIn will be 7 if there are no elements in the queue, indexIn will be the
// same number as indexOut
...
void isr_addDataToAdcBuffer(uint32_t adcData) {
  ...
  if (((adcBuffer.indexIn + INDEX_IN_OFFSET) % ADC_BUFFER_SIZE) ==
      adcBuffer.indexOut) {
...
uint32_t isr_removeDataFromAdcBuffer() {
  ...
  // we decrement the number of elements
    ...
    adcBuffer.indexOut =
        (adcBuffer.indexOut + INDEX_OUT_OFFSET) % ADC_BUFFER_SIZE;
